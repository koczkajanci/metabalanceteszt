export default function CalorieChart({ data = [] }) {
  if (!Array.isArray(data) || data.length === 0) {
    return <div className="cal-chart-placeholder">Nincs adat.</div>;
  }
  ...
  const values = data.map((d) => Number(d.value ?? d.osszeg ?? 0));
  const labels = data.map((d) => String(d.label ?? d.datum ?? ""));
  ...
  return (
    <div className="cal-chart">
      ...
      <div className="cal-chart-legend">Kaloria</div>
    </div>
  );
}
